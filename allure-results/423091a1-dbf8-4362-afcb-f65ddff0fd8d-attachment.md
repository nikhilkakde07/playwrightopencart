...
- Name: apitests\networkMonitoring.spec.ts >> Network monitoring test
- Location: tests\apitests\networkMonitoring.spec.ts:4:5

# Error details

```
Error: locator.click: Error: strict mode violation: getByRole('link', { name: 'iPhone' }) resolved to 4 elements:
    1) <a href="index.php?route=product/product&path=57&product_id=49">…</a> aka getByRole('link', { name: 'iPhone' }).first()
    2) <a href="index.php?route=product/product&path=57&product_id=49">…</a> aka getByRole('link', { name: 'iPhone' }).nth(1)
    3) <a href="https://naveenautomationlabs.com/opencart/index.php?route=product/product&product_id=40">…</a> aka getByRole('link', { name: 'iPhone' }).nth(2)
    4) <a href="https://naveenautomationlabs.com/opencart/index.php?route=product/product&product_id=40">iPhone</a> aka getByText('iPhone', { exact: true })
...
```ts
  ...
  3  | 
  4  | test("Network monitoring test", async({page})=>{
  5  | 
  6  |     page.on('request',async req=>{
  7  |         console.log("outgoing request:", req.method(),req.url());
  8  |     })
  9  |      page.on('response',async res=>{
  10 |         console.log("Incoming request:", res.status(),res.url());
  11 |     })
  12 | 
  13 |     await page.goto("https://naveenautomationlabs.com/opencart/index.php?route=common/home")
> 14 |     await page.getByRole('link',{name: 'iPhone'}).click()
     |                                                   ^ Error: locator.click: Error: strict mode violation: getByRole('link', { name: 'iPhone' }) resolved to 4 elements:
  15 | 
  16 | })
  17 | 
  18 | //npx playwright test tests/networkMonitoring.spec.ts
```